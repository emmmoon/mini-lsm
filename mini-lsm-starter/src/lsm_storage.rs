...
fn key_within(user_key: &[u8], table_begin: KeySlice, table_end: KeySlice) -> bool {
    user_key <= table_end.key_ref() && user_key >= table_begin.key_ref()
}
...
impl LsmStorageInner {
    ...
    pub(crate) fn open(path: impl AsRef<Path>, options: LsmStorageOptions) -> Result<Self> {
        ...
        if !manifest_path.exists() {
            ...
            let (manifest_instance, records) = Manifest::recover(&manifest_path)?;
            ...
            if let CompactionController::Leveled(_) = &compaction_controller {
                for (id, sst) in &mut state.levels {
                    sst.sort_by(|x, y| {
                        state
                            .sstables
                            .get(x)
    ...
    pub fn get_with_ts(&self, _key: &[u8], read_ts: u64) -> Result<Option<Bytes>> {
        ...
        memtable_iters.push(Box::new(memtable));
        for imm in snapshot.imm_memtables.iter() {
            let immetable = imm.scan(
                Bound::Included(KeySlice::from_slice(_key, TS_RANGE_BEGIN)),
                Bound::Included(KeySlice::from_slice(_key, TS_RANGE_END)),
            );
            memtable_iters.push(Box::new(immetable));
        }
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        ...
        self.manifest
            ...
            .context("failed to record new memtable")?;
        self.sync_dir()?;
        Ok(())
    }
    ...
    pub fn force_flush_next_imm_memtable(&self) -> Result<()> {
...

        {
            ...
            if self.compaction_controller.flush_to_l0() {
                ...
                snapshot.levels.insert(0, (sst.sst_id(), vec![sst_id]));
            }
    ...
    /// Create _an iterator over a range of keys.
    pub fn scan_with_ts(
        &self,
        _lower: Bound<&[u8]>,
        _upper: Bound<&[u8]>,
