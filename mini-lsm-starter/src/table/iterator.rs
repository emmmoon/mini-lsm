...
impl SsTableIterator {
    ...
    pub fn create_and_seek_to_key(table: Arc<SsTable>, key: KeySlice) -> Result<Self> {
        let mut index = table.find_block_idx(key);
        let mut blk_iter =
            BlockIterator::create_and_seek_to_key(table.read_block_cached(index)?, key);
        if !blk_iter.is_valid() && index != table.num_of_blocks() - 1 {
            index += 1;
    ...
    pub fn seek_to_key(&mut self, key: KeySlice) -> Result<()> {
        ...
        self.blk_iter =
            BlockIterator::create_and_seek_to_key(self.table.read_block_cached(index)?, key);
